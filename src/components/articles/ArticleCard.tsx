...
function WhatsAppShareBtn({ slug, title }: { slug: string; title: string }) {
  const articleUrl = `https://indlish.com/articles/${slug}`;
  ...
  return (
    <a
      ...
      className="p-1.5 rounded-lg text-green-500 hover:bg-green-500/10 transition-colors opacity-0 group-hover:opacity-100"
    >
    ...
    <article className="card hover:border-neutral-600 transition-colors group">
      ...
      <div className="flex items-center gap-2 mb-3">
        {article.author.image && (
          <img src={article.author.image} alt={article.author.name || ''} loading="lazy" className="w-6 h-6 rounded-full" />
        )}
      ...
      <div className="flex items-center justify-between">
        ...
        <WhatsAppShareBtn slug={article.slug} title={article.title} />
      </div>
